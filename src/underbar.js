(function() {
  ...
  _.reject = function(collection, test) {
    var oppositeTest = function(elem) {
      if (test(elem)) {
        return false;
      } else {return true;}
    };
  ...
  _.memoize = function(func) {
    ...
    return function() {
      var inputString = JSON.stringify(Array.from(arguments));
      if (_.contains(args, inputString)) {
        return results[_.indexOf(results, inputString)];
      } else {
  ...
  _.shuffle = function(array) {
    var shuffledArray = Array.from(array);
    
    var n = array.length, i, temp;
    while (n) {
      i = Math.floor(Math.random() * n);
      n--;
      temp = shuffledArray[n];
  ...
  _.invoke = function(collection, functionOrKey, args) {
    var invoked = [];
    _.each(collection, function(item) {
      invoked.push(functionOrKey(item));
      // if type is function, use .apply
      // else use .property lookup
    });
    return invoked;
  };
